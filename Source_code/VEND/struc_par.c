...
//------------------------------------------------
   ...
   if(strstr( name, "INS") != NULL ) return 14;
//------------------------------------------------
   if(strstr( name, "XYZ") != NULL ) return 15;
   if(strstr( name, "xyz") != NULL ) return 15;
//------------------------------------------------
   if(strstr( name, "DAT") != NULL ) return 17;
   if(strstr( name, "dat") != NULL ) return 17;
//------------------------------------------------
...
//				lAtom[j].Biso = 1;
			...
			while(lineID == 0)
			{
				...
				if (lineID == 0) {
					...
					while( token != NULL )
					{
						if (j == 0) sprintf(cell_param,"%s",token);
						if (j > l) sscanf(token,"%f",&B[j-l-1]);
						token = strtok( NULL, seps );
						j++;
					}
					for(j=0; j<nAtom; j++){
						if (strstr(cell_param,lAtom[j].name) != NULL){
...
int readSCATfile(char *fname)
{
	...
	while (lineID == 0)
	{
		...
		if (pos[0] == '-') lineID = 1;
			...
			sprintf(lAtom[i].simb,"");
			sprintf(lAtom[i].name,"");
			sscanf(line,"%i %i %f %f %f",&atomID,&Dum,&lAtom[i].x,&lAtom[i].y,&lAtom[i].z);
			lAtom[i].ex = lAtom[i].ey = lAtom[i].ez = 0;
			lAtom[i].g = 1;
			for(j=0; j<6; j++) lAtom[i].B[j] = 0;
			lAtom[i].Biso = 1;
...
//------------------------------------------------------ x,y,z
				...
				lAtom[j+3*I1].g = 1;
				lAtom[j+3*I1].B[0] = lAtom[j+3*I1].B[1] = lAtom[j+3*I1].B[2] = 1;
...
//----------------------------------------------- XMol (XYZ)
		case(15):{bonds_type = 0; radii_type = 0; FileControl = readXYZfile(fname); break;}
//----------------------------------------------- XMol (XYZ)
		case(16):{bonds_type = 0; radii_type = 0; FileControl = readSCATfile(fname); break;}
//----------------------------------------------- MXD
		case(17):{bonds_type = 0; radii_type = 0; FileControl = readMXDfile(fname); break;}
		}
		if (FileControl == 0)  {
			fog_color[0] = BKCL[0];
			fog_color[1] = BKCL[1];
			...
			if (use_aniso == 1) {
				for(i=0; i<nAtom; i++) {
					if ((lAtom[i].B[0] == 0) || (lAtom[i].B[1] == 0) || (lAtom[i].B[2] == 0)){
						lAtom[i].B[0] = lAtom[i].B[1] = lAtom[i].B[2] = 0.001f;
						lAtom[i].B[3] = lAtom[i].B[4] = lAtom[i].B[5] = 0;
					}
				}
			}
...
//--------------------------------------------------------------------------- Anizotropic Thermal Parameters
	if (export_mode == 2){
		j = 0;
		for(i=0; i<nAtom; i++){if ((lAtom[i].Biso == 0) || (lAtom[i].Biso == 1)) j++;}
		if(j > 0){
			fprintf(fptr,"\nloop_\n");
			...
			for(i=0; i<nAtom; i++){
				if ((lAtom[i].Biso == 0) || (lAtom[i].Biso == 1)){
...
//------------------------------------------ Reade lines from template file
	...
	while(!feof(tfptr)){
		...
		if (code_line == 0) fputs(line, fptr);
			...
			case(2):{
				sprintf(string,"%s","");
				sscanf(Title,"%s",&string);
//				fprintf(fptr,"PHNAME1 = '%s': Phase name (CHARACTER*25).\n",string);
				fprintf(fptr,"PHNAME1 = '%s': Phase name (CHARACTER*25).\n",Title);
				break;
					}
			case(3):{
				...
				while(code_line != 0){
					...
					if (line[0] == '}') {
						...
						case(2):{
							...
							bt = cCell.a[4];
							gm = cCell.a[5];
							vol = (float)(a*b*c*sqrt(1 - cos(al*PI/180)*cos(al*PI/180) 
					               - cos(bt*PI/180)*cos(bt*PI/180)
								   - cos(gm*PI/180)*cos(gm*PI/180)
								   + 2*cos(al*PI/180)*cos(bt*PI/180)*cos(gm*PI/180)));
							ar = (float)(b*c*sin(al*PI/180)/vol);
							br = (float)(a*c*sin(bt*PI/180)/vol);
							cr = (float)(a*b*sin(gm*PI/180)/vol);
							for(i=0; i<nAtom; i++){
								if (((lAtom[i].B[0] == 1) || (lAtom[i].B[0] == 0)) && 
									((lAtom[i].B[1] == 1) || (lAtom[i].B[1] == 0)) && 
									((lAtom[i].B[2] == 1) || (lAtom[i].B[2] == 0)) &&
									(lAtom[i].B[3] == 0) && (lAtom[i].B[4] == 0) && (lAtom[i].B[5] == 0)) {
									fprintf(fptr,"%s/%s  %5.3f %8.5f %8.5f %8.5f %6.3f  00000\n",
										    lAtom[i].name,lAtom[i].simb,lAtom[i].g,
											lAtom[i].x,lAtom[i].y,lAtom[i].z,lAtom[i].Biso);
								}
								else {
									if (Btype == 1) {
										b11 = lAtom[i].B[0];
										...
										b11 = (float)(2*PI*PI*ar*ar*lAtom[i].B[0]);
										b22 = (float)(2*PI*PI*br*br*lAtom[i].B[1]);
										b33 = (float)(2*PI*PI*cr*cr*lAtom[i].B[2]);
										b12 = (float)(2*PI*PI*ar*br*lAtom[i].B[3]);
										b13 = (float)(2*PI*PI*ar*cr*lAtom[i].B[4]);
										b23 = (float)(2*PI*PI*br*cr*lAtom[i].B[5]);
									}
