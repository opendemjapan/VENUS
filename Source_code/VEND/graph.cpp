...
GLUI_Panel *exp_from_panel; 
GLUI_Checkbox *exp_from_G;
GLUI_Checkbox *exp_from_2D;
GLUI_Checkbox *exp_from_3D;
...
/**************************************** get_export_param() ****************/
void get_export_param(int &smoothing, int &scale, int &exp_fr)
{
...
void export_control_cb( int control )
{
	...
	case(21):
	case(22):{
		exp_from_ID = control - 20;
		switch(exp_from_ID){
		case(0):{
			exp_from_G->set_int_val(1);
			exp_from_2D->set_int_val(0);
			exp_from_3D->set_int_val(0);
			break;
				}
		case(1):{
			exp_from_G->set_int_val(0);
			exp_from_2D->set_int_val(1);
			exp_from_3D->set_int_val(0);
			break;
				}
		case(2):{
...
/**************************************** init_Export_Dialog ****************/
void set_Export_param(char *name, int format, int ID_2D, int ID_3D)
{
	sprintf(e_name,"");
	...
	case(1):
		...
		break;
	case(2):
		sprintf(e_format_name,"JPEG");
		break;
	case(3):
		sprintf(e_format_name,"JPEG 2000");
		break;
	case(4):
...
void init_Export_Dialog()
{
	...
	exp_from_ID = 0;
	if ((exp_2D_ID == -1) && (exp_3D_ID == -1)) exp_from_G->disable();
	else exp_from_G->enable();
	if (exp_2D_ID == -1) exp_from_2D->disable();
	else exp_from_2D->enable();
	if (exp_3D_ID == -1) exp_from_3D->disable();
	else exp_from_3D->enable();
	exp_from_G->set_int_val(1);
}
...
//	expglui->add_column_to_panel(exp_panel,true);
	...
	exp_from_panel = expglui->add_panel_to_panel(exp_panel,"Export from",GLUI_PANEL_EMBOSSED);
	exp_from_panel->set_alignment(GLUI_ALIGN_CENTER);

	exp_from_G  = expglui->add_checkbox_to_panel(exp_from_panel,"Graphics Window",NULL,20,export_control_cb);
	exp_from_2D = expglui->add_checkbox_to_panel(exp_from_panel,"2D Map Window",NULL,21,export_control_cb);
	exp_from_3D = expglui->add_checkbox_to_panel(exp_from_panel,"Bird's-Eye View Window",NULL,22,export_control_cb);
...
/**************************************** Msg_Export_Dialog() ***************/
void Msg_Export_Dialog(int ID, int mode)
{
	msg_Image_ID = -1;
	...
	case(1):{
		mline1 = msgglui->add_statictext_to_panel(msg_panel,"A graphics file from the 2D Map Window");
		break;
			}
	case(2):{
		mline1 = msgglui->add_statictext_to_panel(msg_panel,"A graphics file from the Bird's-Eye View Window");
		break;
			}
	}
